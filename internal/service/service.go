...
func (s *Service) ReadCSVFile() ([]models.PartnerPayment, error) {
	filePath := "./files/alif/input/input_22.06.2023.csv"
...
func (s *Service) GetAllPartners() (partners []models.Partner, err error) {
	...
	for _, partner := range partners {
		switch partner.Name {
		case BankALIF:
			_, err := s.CheckInterval(partner)
			if err != nil {
				logger.Errorf("can not get integration date from %v", partner.Name)
			}
		case BankDC:
			_, err := s.CheckInterval(partner)
			if err != nil {
	...
	// 4. Create registers from interval_date until curr_time depending from interval_type/ i realized it in 2 step
...
func (s *Service) CheckInterval(partner models.Partner) (*TimeS, error) {
	currentTime := time.Now()

	parseIntegrationDate, err := time.Parse("2006-01-02", partner.IntegrationDate)
	if err != nil {
